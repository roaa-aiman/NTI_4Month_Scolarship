...
void PWM_voidInitializeChannel_0(void)
{
    ...
    /*Set Fast and phase correct mode*/
	...
	#if TIMER0_PHASE_CORRECT_PWM_MODE_SELECT == TIMER0_NORMAL_MODE_PHASE_CORRECT_PWM
	CLR_BIT(TCCR0_REG, TCCR0_COM00);
	CLR_BIT(TCCR0_REG, TCCR0_COM01);
	#elif TIMER0_PHASE_CORRECT_PWM_MODE_SELECT == TIMER0_CLEAR_ON_COMPARE_PHASE_CORRECT_PWM
	CLR_BIT(TCCR0_REG, TCCR0_COM00);
	SET_BIT(TCCR0_REG, TCCR0_COM01);
	#elif TIMER0_PHASE_CORRECT_PWM_MODE_SELECT == TIMER0_SET_ON_COMPARE_PHASE_CORRECT_PWM
	SET_BIT(TCCR0_REG, TCCR0_COM00);
	SET_BIT(TCCR0_REG, TCCR0_COM01);
	#else
	//error
...
void PWM_voidGenerateChannel_0 (u8 copy_u8DutyCycle)
{
	...
	{
		#if TIMER0_MODE == TIMER0_PHASE_CORRECT_PWM_MODE
		  #if TIMER0_FAST_PWM_MODE_SELECT == TIMER0_CLEAR_ON_COMPARE_FAST_PWM
		  OCR0_REG = (((u16)copy_u8DutyCycle*510)/200)-1;
		  #elif TIMER0_FAST_PWM_MODE_SELECT == TIMER0_SET_ON_COMPARE_FAST_PWM
		  OCR0_REG = ((510-(((u16)copy_u8DutyCycle*510)/100))/2)-1;
		  #else
		  //error
		  #endif
		#elif TIMER0_MODE == TIMER0_FAST_PWM_MODE
		  #if TIMER0_PHASE_CORRECT_PWM_MODE_SELECT == TIMER0_CLEAR_ON_COMPARE_PHASE_CORRECT_PWM
